...
2. Locate the collection you wish to export.
    
    In this example, you will use the **Grouping Requests** collection which you created in [Grouping Requests with Collections](Task3.md/#set-up).

    ![Collections Sidebar](assets/task4/img_1.png "Open the Collections panel in the sidebar")

3. Hover over the collection name until the **"⋯"** icon appears.

    ![Collection Options Menu](assets/task4/img_2.png "Hover to reveal the collection options menu")

4. Click the icon to open the context menu.
5. Hover over **"More"** and select **"Export collection"**.

    ![Export Collection Option](assets/task4/img_3.png "Open the Export option from the collection menu")

6. Click **"Export JSON"** in the bottom-right corner of the dialog that appears.

    ![Export Button Dialog](assets/task4/img_4.png "Confirm export to download the collection JSON file")

7. Select a destination folder to save the file.
8. Click **"Save"**.
...
3. Click the **"files"** link in the dialog that appears to import from a local file.

    ![Import Files Option](assets/task4/img_7.png "Select files to import a collection from your computer")

4. Navigate to the folder containing your `.json` file
5. Select it, and click **"Open"**.

    ![Select JSON File](assets/task4/img_8.png "Choose the exported Postman collection JSON file")

6. Click **"Import as Copy"** to import it without overwriting the existing collection.
...
7. Check the collection that now appears in your **"COLLECTIONS"** sidebar, ready for use.
